...
public class register_servlet extends HttpServlet {
    ...
    // Patterns for validating account number (at least 6 digits) and phone number (exactly 10 digits)
    private static final Pattern ACCOUNT_PATTERN = Pattern.compile("\\d{6,}");
    private static final Pattern PHONE_PATTERN = Pattern.compile("\\d{10}");

    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        // Collect form parameters from the registration form
        ...
        // Set values back to request attributes in case form needs to be re-displayed (sticky form)
        ...
        boolean hasError = false; // Flag to track validation errors
        ...
        // If any validation error exists, return to registration page
        if (hasError) {
        ...
        try {
            ...
            // If duplicates exist, rollback transaction and return to form
            if (hasError) {
                con.rollback();
                req.setAttribute("general_error", "Please fix the highlighted errors.");
                req.getRequestDispatcher("register.jsp").forward(req, resp);
            ...
            String hashed = PasswordUtil.hashPassword(password); // Hash the password before storing
            user.setPasswordHash(hashed);
            user.setRole("customer"); // Default role
            user.setStatus("Active"); // Default status

            // Save user to database and retrieve generated user ID
            int newUserId = userDao.createUser(user);

            // ===========================
            // Create new Customer entity linked to user
            // ===========================
            ...
            try {
                if (con != null)
                    con.rollback(); // Rollback on any exception
